...
A default-`vm` counterpart case measures the same two mutation attempts in its own process, so the pair reads as one comparison. The vm evaluator accepts both: the write to the Package's own sandbox realm stays inside that realm, while a write that reaches the Host realm through an endowed-function constructor escape survives `stop` and `undefine` and is then observable from the test process and from a later Package that only receives an ordinary Host array. Argument cases on both evaluators measure that a Host half mutates the object its caller passed to `harness.handle`, because SES hardens endowments and the returned Plugin rather than inbound arguments.
