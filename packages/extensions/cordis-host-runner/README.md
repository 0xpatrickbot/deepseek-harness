...
The host half of model-mounted dynamic packages: the definition registry, the baseline `node:vm` evaluator, an experimental SES evaluator, the fiber lifecycle for host halves, the invoke handler table, and the run round trip a browser page carries out. Provided as `ctx.dynamicCordisRunner`. The model-facing tools live in [`@deepseek-ai/dsh-tool-cordis`](../tool-cordis/README.md); the browser half is loaded by [`@deepseek-ai/dsh-cordis-client-runner`](../cordis-client-runner/README.md).

## What it does

Two phases: `define` only records, and everything with an effect hangs off a run.

- `define` / `undefine` own a definition's life. `define` trims and requires the metadata, prechecks each half's JavaScript syntax by compiling it (running nothing), mints `dyn-<n>`, and records the definition against the session that asked — it has no effect to roll back, so syntactically invalid code is refused before an id exists. Evaluator-specific source restrictions are enforced later during `run`, so syntactically valid code can still fail Host activation. `undefine` stops a running definition first, then forgets it. Neither crosses the wire: only the model's own tool call defines.
- `run` answers the model's request to run one definition, and its two shapes differ by whose business the package is. A host-only package is this process's own: the host half is evaluated by the configured Host evaluator under the `cordis-dynamic` group fiber and the call returns. A package with a browser half has to be carried out by a page, so `run` becomes an answerable round trip — it emits `cordis/request-run`, suspends, and is settled by a person allowing or declining it. There is no timer; the caller's `AbortSignal` (the asking turn was cancelled) is the only other way out, and it announces the cancellation so other pages stop offering an answer. Whether any page will answer is not knowable when the request is sent — a page that received it may still never answer, so a deployment with no page connected suspends like any other unanswered request and ends in `cancelled`. `run` has no wire face — `cordis_run` calls it in process.
...
The default `vm` evaluator isolates globals but is not a security boundary: data-valued Node globals are absent, calls to `require`, `fetch`, and timer globals receive teaching redirects to Cordis services, and Host-realm helpers remain reachable. The experimental `ses` evaluator runs process-global `lockdown()` before its first component, creates a fresh `Compartment` for each activation, and endows only a hardened tagged console, `harness.defineTool`, `harness.registerTool`, `harness.handle`, `btoa`, `atob`, `TextEncoder`, and `TextDecoder`. It validates and hardens the returned Plugin before handing it to the same guard and `cordis-dynamic` fiber lifecycle. This proof of concept restricts ambient reachability for code evaluated on that path; it is not production containment and does not cover static Plugins, browser code, or other JavaScript in the process.

Both evaluators give the Plugin the existing Context façade. That façade permits optional undeclared lookup through `ctx.get(name)`, and a returned Service value can carry nested authority that its top-level guard does not enumerate. Every reachable Service still acts on the live runtime. See the [experimental SES evaluator Agent Note](../../../.agents/notes/implemented/architecture/2026-08-14-experimental-ses-host-evaluator.md) and the [self-referential toolset Agent Note](../../../.agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md).

## Config

| Field | Default | Meaning |
|---|---|---|
| `experimentalHostEvaluator` | `vm` | `vm` keeps the baseline evaluator; `ses` selects the experimental process-global lockdown and per-activation Compartment path |
| `vmTimeoutMs` | `5000` | Milliseconds the synchronous portion of a Host half may run in `vm`; ignored by `ses` |

The evaluator fields do not set a deadline for a run request that waits for a person; that round trip still has no deadline of its own.
...
Nothing directly: this package registers no tool and injects no prompt. Its refusals reach the model through the `cordis_*` tool results that call it — an unparseable half names the offending line, an SES source-censor refusal names activation as the later check and tells the author which raw token to remove, a missing definition explains that definitions live in memory only, a `rejected` or `cancelled` run reports that a person declined or the turn ended rather than that anything failed, and a failed browser-half load carries the answering page's own error text.
...
- `vmTimeoutMs` bounds only synchronous `vm` evaluation, and an async Host-half body escapes it. `Compartment.evaluate` has no `node:vm` synchronous timeout equivalent, so the SES path ignores `vmTimeoutMs`; a component can monopolize the shared thread or heap. The experiment makes no denial-of-service or availability-isolation claim.
- SES lockdown is process-global and irreversible. The current assembled headless profile activates and tears down successfully after lockdown, but a deployment must test every process-local Plugin and dependency it loads. Hardening the endowed Host `TextEncoder` and `TextDecoder` also freezes those constructors and prototypes process-wide. SES integration tests therefore run in isolated subprocesses instead of locking down the main test runner.
- SES 2.3.0 conservatively scans raw source during activation and rejects apparent `import(...)` expressions and HTML-comment tokens (`<!--` or `-->`) even inside strings and comments. The define-time VM compilation checks JavaScript syntax only, so these parseable cases receive an actionable `host-half-failed` result during `run`; module loading remains unsupported.
- SES shared intrinsics make `Date.now()`, `new Date()`, and `Math.random()` throw secure-mode `TypeError`s. SES also omits the VM teaching redirects: `typeof require`, `typeof fetch`, and `typeof setTimeout` are `undefined`, while calling them reports that the name is not a function. Use declared Cordis filesystem, Web, process, and timer Services instead; the evaluator adds no compatibility endowments.
- SES error taming preserves actionable syntax and runtime messages but may redact or omit stacks. After lockdown, the existing vm syntax precheck may also fall back from its TypeScript-specific teaching text to the generic async-function-body and bracket-balance hint because the inspected stack no longer carries the offending source line.
- The Context façade still permits optional undeclared Service lookup through `ctx.get(name)`, and a Service return can convey nested authority. SES hardens the façade and Plugin surface; it does not prove that reachable Service object graphs contain no additional authority.
