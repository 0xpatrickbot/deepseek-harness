...
const requireBuiltArtifacts = process.env.DSH_EXAMPLE_MODE === 'lib'
 ...
 */
describe.skipIf(!requireBuiltArtifacts && missingArtifacts.length > 0)(
  'experimental SES Host evaluator through the built product profile',
  () => {
    it('defines, activates, calls, stops, and tears down an SES package', async () => {
      expect(missingArtifacts, 'built-mode SES smoke is missing required artifacts; run pnpm build').toEqual([])
